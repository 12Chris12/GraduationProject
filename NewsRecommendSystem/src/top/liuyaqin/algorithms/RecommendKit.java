 ...
 * 
 */
package top.liuyaqin.algorithms;
...
import top.liuyaqin.contentbasedrecommend.CustomizedHashMap;
...
public class RecommendKit
{
	...
	public static void filterBrowsedNews(Collection<Long> col, Long userId)
	{
		try
		{
			List<Newslogs> newslogsList = Newslogs.dao.find("select joke_id from y_browse_record where user_id=?",userId);
			for (Newslogs newslog:newslogsList)
			{
				if (col.contains(Long.parseLong(newslog.getJokeId())))
				{
					col.remove(Long.parseLong(newslog.getJokeId()));
				}
			}
		}
		catch (Exception e)
...
//			List<Recommendations> recommendationList = Recommendations.dao.find("select news_id from recommendations where user_id=? and derive_time>?",userId,getInRecDate());
			//---------------lyq rectify
//			System.out.println("过滤已被推荐的新闻");
			List<Recommendations> recommendationList = Recommendations.dao.find("select jokeId from y_recommend where userId=?",userId);
			for (Recommendations recommendation:recommendationList)
			{
				if (col.contains(Long.parseLong(recommendation.getJokeId())))
				{
					col.remove(Long.parseLong(recommendation.getJokeId()));
				}
			}
		}
		catch (Exception e)
	...
	{
		System.out.println("before_Days is "+before_Days);
		SimpleDateFormat date_format = new SimpleDateFormat("yyyy-MM-dd");
		Calendar calendar = Calendar.getInstance(); // 得到日历
		calendar.add(Calendar.DAY_OF_MONTH, before_Days); // 设置为前beforeNum天
		Date d = calendar.getTime();
//		System.out.println("仅分析在该日期之后的新闻浏览记录"+date_format.format(d));
...
//			System.out.println("userList"+userPrefListQuery);
			if (!userPrefListQuery.equals("()"))
			{
				List<Users> userList = Users.dao.find("select user_id,token from y_user where user_id in " + userPrefListQuery);
				userPrefListMap = new HashMap<Long, CustomizedHashMap<Integer, CustomizedHashMap<String, Double>>>();
				for (Users user:userList)
				{
//					System.out.println(JsonKit.jsonPrefListtoMap(user.getToken()));
					userPrefListMap.put(user.getUserId(), JsonKit.jsonPrefListtoMap(user.getToken()));
				}
			}
		}
		catch (Exception e)
...
//			System.out.println(ite.next());
	...
	public static void insertRecommend(Long userId, Iterator<Long> newsIte, int recAlgo)
	{
		try
		{
			while (newsIte.hasNext())
			{
				...
				rec.setJokeId(newsIte.next().toString());
				rec.setFeedback(0);     //0表示给用户推荐了，用户未浏览
				rec.save();
			}
		}
		catch (Exception e)
	...
	public static List<Long> getActiveUsers()
	{
		try
		{
			...
			List<Users> userList=Users.dao.find("select distinct user_id,name from y_user where last_login_time>" + getInRecDate(activeDay));
