 ...
 * 
 */
package top.liuyaqin.contentbasedrecommend;
...
import top.liuyaqin.algorithms.JsonKit;
...
{	
	...
	//每日衰减系数
	private static final double DEC_CODE=0.6;
...
//			System.out.println("userPrefListMap is "+userPrefListMap.keySet());
//			System.out.println("newsTFIDFMap is "+newsTFIDFMap.keySet());
//			System.out.println("userBrowsedMap is "+userBrowsedMap.keySet());
...
//				System.out.println(newsList);
				for(Long news:newsList){
					Integer moduleId=(Integer) newsTFIDFMap.get(news+"category");
					//获得对应模块的（关键词：喜好）map

					CustomizedHashMap<String,Double> rateMap=userPrefListMap.get(userId).get(moduleId);

					//-----------lyq修改 如果user表中缺乏该模块 新建一个模块----------解决空指针异常-----------
					if(rateMap==null) rateMap = new CustomizedHashMap<>();
					//------lyq rateMap的获取有问题 2021.5.3 17:58x
//					System.out.println(news+"的 rateMap is "+rateMap);
					...
					//------lyq---存入数据库里的tf-idf值保留两位小数
					DecimalFormat df = new DecimalFormat("#.00");
//					System.out.println("现在分析joke_id为"+news+"的新闻");
					while(keywordIte.hasNext()){
						Keyword keyword=keywordIte.next();
						String name=keyword.getName();
						if(rateMap.containsKey(name)){
							rateMap.put(name, Double.valueOf(df.format(rateMap.get(name)+keyword.getScore())));
						}
						else{
							rateMap.put(name, Double.valueOf(df.format(keyword.getScore())));
						}
					}
			...
			while(iterator.hasNext()){
				...
				try
				{
					Db.update("update y_user set token='"+userPrefListMap.get(userId)+"' where user_Id=?",userId);
				}
				catch (Exception e)
				{
					e.printStackTrace();
	...
	public void autoDecRefresh(Collection<Long> userIdsCol){
		try
		{
			...
			List<Users> userList=Users.dao.find("select user_id,token from y_user where user_id in "+inQuery);
			//用以更新的用户喜好关键词map的json串
			//用于删除喜好值过低的关键词
			ArrayList<String> keywordToDelete=new ArrayList<String>();
			for(Users user:userList){
				String newPrefList="{";
				HashMap<Integer,CustomizedHashMap<String,Double>> map=JsonKit.jsonPrefListtoMap(user.getToken());
				...
				while(ite.hasNext()){
					...
					if(!(moduleMap.toString().equals("{}"))){
						...
						while(inIte.hasNext()){
							...
							moduleMap.put(key,Double.valueOf(df.format(result)));
						}
					}
				...
				Db.update("update y_user set token="+newPrefList+" where user_id=?",user.getUserId());
			}
		}
		catch (Exception e)
		{
	...
	private HashMap<Long,ArrayList<Long>> getBrowsedHistoryMap(){
		...
		try
		{
			//----lyq 修改 分析用户浏览记录时不考虑before_days
//			List<Newslogs> newslogsList=Newslogs.dao.find("select * from y_browse_record where view_time>"+RecommendKit.getSpecificDayFormat(5));
			List<Newslogs> newslogsList=Newslogs.dao.find("select * from y_browse_record");
			for(Newslogs newslogs:newslogsList){
				if(userBrowsedMap.containsKey(Long.parseLong(newslogs.getUserId()))){
					userBrowsedMap.get(Long.parseLong(newslogs.getUserId())).add(Long.parseLong(newslogs.getJokeId()));
				}
				else{
					userBrowsedMap.put(Long.parseLong(newslogs.getUserId()), new ArrayList<Long>());
					userBrowsedMap.get(Long.parseLong(newslogs.getUserId())).add(Long.parseLong(newslogs.getJokeId()));
				}
			}
		}
...
//				System.out.println("newsIdListQuery:"+newsIdLitQuery);
...
//			}
			...
			newsIdListQuery=newsIdListQuery.substring(0, newsIdListQuery.length()-1)+")";
			//提取出所有新闻的关键词列表及对应TF-IDf值，并放入一个map中
			List<News> newsList=News.dao.find("select joke_id,title,contentHtml,category from y_joke where joke_id in "+newsIdListQuery);
			for(News news:newsList){
				newsTFIDFMap.put(String.valueOf(news.getJokeId()), TFIDF.getTFIDE(news.getTitle(), news.getContent(),KEY_WORDS_NUM));
				newsTFIDFMap.put(news.getJokeId()+"category", news.getCategory());
			}
		}
		catch (Exception e)
		{
